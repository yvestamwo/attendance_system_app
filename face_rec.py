...
hostname = 'redis-16878.c325.us-east-1-4.ec2.cloud.redislabs.com'
portnumber = 16878
password = 'j5jYqt7kvNXVsJmbt0OH6KGLQI80GcNv'
...
def retrive_data(name):
    ...
    #retrive_series
    #retrive_df = retrive_series.to_frame()
    #retrive_df
...
faceapp = FaceAnalysis(name = 'buffalo_sc', 
                       ...
                       provideds= ['CPUExecutionProvider'])
faceapp.prepare(ctx_id=0, det_size=(640,640), det_thresh=0.5)


#ML Search Algorithm
def ml_search_algorithm(dataframe, facial_features, test_vector,name_role = ['Name', 'Role'], thresh=0.5):
...
class realtimePred:
    ...
    def saveLogs_redis(self):
            ...
            #step-2: drop the duplicated iformation(name)

            dataframe.drop_duplicates('name', inplace=True)
            #step-3: push date to redis database (list)
            # encode the data
            ...
            for name, role, ctime in zip(name_list, role_list, ctime_list):
                if name != 'Unknown':
                    ...
                    encoder_data.append(concat_string)
            if len(encoder_data)>0:
                r.lpush('attendance:logs', *encoder_data)
...
class RegistrationForm:
    ...
    def get_embedding(self, frame):
        ...
        results = faceapp.get(frame, max_num=1) # set to 1 in order to get information for only one face and 0 for multipe.
        embeddings = None
        for res in results:
            self.sample += 1
            x1, y1, x2, y2 = res['bbox'].astype(int)
            cv2.rectangle(frame, (x1,y1),(x2,y2),(0,255,0),1)
            # Put test sample info
